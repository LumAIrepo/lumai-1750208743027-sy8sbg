  ...
  useEffect(() => {
    const mockStreams: Stream[] = [
      ...
      },
      {
        ...
        sender: '8P3p3dBbEEmZ5ccB2DeqfNtzk2p4o6BSTAfD9nN8Lc95',
      ...
      },
      {
        ...
        recipient: '2WmzNHdNxbC6t4cy6dqNR3P6T8pwjkfQP5VUagrbRzSn',
      ...
      },
      {
        ...
        sender: '5P3p3dBbEEmZ5ccB2DeqfNtzk2p4o6BSTAfD9nN8Lc95',
    ...
    const newStats: DashboardStats = {
      totalStreaming: mockStreams
        .filter(s => s.type === 'outgoing' && s.status === 'active')
        .reduce((sum, s) => sum + s.flowRate, 0),
      totalReceiving: mockStreams
        .filter(s => s.type === 'incoming' && s.status === 'active')
        .reduce((sum, s) => sum + s.flowRate, 0),
      activeStreams: mockStreams.filter(s => s.status === 'active').length,
      completedStreams: mockStreams.filter(s => s.status === 'completed').length
    }
    setStats(newStats)
  }, [])
  ...
  const handleStreamAction = (streamId: string, action: 'pause' | 'resume' | 'cancel') => {
    setStreams(prev => prev.map(stream => {
      if (stream.id === streamId) {
        if (action === 'pause') {
          return { ...stream, status: 'paused' as const, flowRate: 0 }
        } else if (action === 'resume') {
          return { ...stream, status: 'active' as const, flowRate: 137 }
        } else if (action === 'cancel') {
          return { ...stream, status: 'cancelled' as const, flowRate: 0 }
        }
      }
      return stream
    }))
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <Card className="bg-slate-800 border-slate-700">
            ...
            <CardContent>
              ...
              <p className="text-xs text-slate-400">tokens per day</p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-slate-800 border-slate-700">
            ...
            <CardContent>
              ...
              <p className="text-xs text-slate-400">streams finished</p>
            </CardContent>
          </Card>
        </div>
          ...
          <CardHeader>
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className="bg-slate-700 border-slate-600">
                <TabsTrigger value="all" className="data-[state=active]:bg-slate-600">All Streams</TabsTrigger>
